    ...
    <div class="flex justify-around items-center h-16 max-w-lg mx-auto px-4">
        ...
        @foreach($items as $item)
            ...
            <a href="{{ route($item['route']) }}"
               @class([
                    'flex flex-col items-center gap-1 text-xs font-medium transition-colors duration-150 text-dark',
                    'hover:text-primary gap-2' => !$active,
               ])>
                <span @class([
                    'flex items-center rounded-full transition-colors',
                    'bg-secondary text-light px-3 py-1' => $active,
                ])>
